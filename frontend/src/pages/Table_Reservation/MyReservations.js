...
const MyReservations = () => {
    ...
    const fetchReservationRatings = async (reservationData) => {
        const pastReservations = reservationData.filter(res => 
        ...
        for (const reservation of pastReservations) {
            try {
                ...
                console.error(`Error fetching rating for reservation ${reservation.id}:`, err);
            }
        }
                    ...
                    <div>
                        ...
                        {isRated && (
                            <div className="mt-2">
                                ...
                                <StarRating initialRating={reservationRatings[reservation.id]} readOnly={true} />
                            </div>
                        )}
                    </div>
    ...
    return (
        <div className="min-h-screen w-full bg-white bg-opacity-90 flex flex-col items-center pt-16 pb-16">
            ...
            <RatingModal 
                ...
                onRatingSuccess={(rating) => handleRatingSuccess(selectedReservationForRating, rating)}
            />
        </div>
    );
};
